            ...
            let parsed = match term.to_ascii_uppercase().as_str() {
                ...
                _ => {
                    let index = term.parse::<usize>().map_err(|err| {
                        format!("unsupported affinity term {term:?}; expected P, E, LPE, C, or a logical processor index: {err}")
                    })?;
...
pub struct AffinityExpressionConfig(pub AffinityExpression);

impl TryFrom<String> for AffinityExpressionConfig {
    type Error = String;

    fn try_from(value: String) -> Result<Self, Self::Error> {
        AffinityExpression::parse(value).map(Self)
    }
}

#[derive(Debug, Deserialize)]
#[serde(untagged)]
pub enum AffinityMaskConfig {
    Integer(u64),
    String(String),
}

impl AffinityMaskConfig {
    pub fn parse(self) -> Result<usize, String> {
        let mask = match self {
            Self::Integer(value) => value,
            Self::String(value) => parse_affinity_mask(&value)?,
        };

        if mask == 0 {
            return Err("affinity_mask must not be zero".to_string());
        }
        usize::try_from(mask).map_err(|_| format!("affinity_mask 0x{mask:x} is too large"))
    }
}

pub fn parse_affinity_mask(value: &str) -> Result<u64, String> {
    let value = value.trim();
    if value.is_empty() {
        return Err("affinity_mask string must not be empty".to_string());
    }
        ...
        .or_else(|| value.strip_prefix("0X"))
    {
        u64::from_str_radix(hex, 16)
            .map_err(|err| format!("invalid hexadecimal affinity_mask {value:?}: {err}"))
    } else {
        value
            .parse::<u64>()
            .map_err(|err| format!("invalid decimal affinity_mask {value:?}: {err}"))
    }
}
...
mod tests {
    ...
    #[test]
    fn affinity_expression_accepts_logical_index_sum() {
    ...
    fn affinity_expression_skips_numeric_terms_outside_usable_cpu_sets() {
        let expression = AffinityExpression::parse("0+1+8".to_string()).unwrap();
        let entries = (0..4).map(|index| cpu_set(index, 0)).collect::<Vec<_>>();
    ...
    fn affinity_expression_returns_none_for_empty_numeric_result() {
        ...
        let unavailable = AffinityExpression::parse("8+9".to_string()).unwrap();
